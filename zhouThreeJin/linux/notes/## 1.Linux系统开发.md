...
![user_config1](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\user_config1.png)
 分为7个字段
...
![user_config2](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\user_config2.png)
只了解前两段 	第一段为用户名 第二段为加密的密码一般为$6$开头可以防止相同密码别人破解 

/etc/group(配置和用户组相关的
![user_config3](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\user_config3.png))
root (用户组名称) :x(是否需要密码，很少使用) : 0（gid）:（组成员列表)就是当前组的附加成员可以给予额外的权限

####  3.查看文件权限的方法
##### 1.文件的基本概念
![dirct](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\dirct.png))
...
![chmod_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\chmod_order.png))
`chmod u+x (755)	/xxx/xxx`
`chown+属主+文件 	更改属主`
`chown+：属组+文件	更改属组`
`chown +属主：属组 +文件  达到属主和属组同时修改`
`chgrp	可以单独更改属组`
![chown_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\chown_order.png))
...
文件的所属主相当于文件的创始人的、而所属组相当于参观者、其他人相当于无关人员。基于这个思想理解就很明白了
 ![chown_chmod](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\chown_chmod.png)
...
这种网络接口的名称不统一很难进行批量操作，所以需要修改回网卡的名字为eth0的格式
 ![ifconfig_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\ifconfig_order.png)
这些是受到biosdevname和net.ifnames两个参数的影响，他们被配置在
/etc/default/grub文件中，GRUB_CMDLINE_LINUX里面增加biosdevname=0  net.ifname=0
再更新文件grub 
...
route -n 查看网关  -n不解析主机名 
 ![route_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\route_order.png)
2.iproute2
这个不详细介绍
...
ifconfig 	eth0 	10.211.55.4 	netmask 255.255.255.0
 ![ifconfig_1_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\ifconfig_1_order.png)
...
查看默认网关
route -n
 ![route_n_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\route_n_order.png)
Destination 表示目的ip
...
route add 0.0.0.0 gw 10.211.55.1
 ![route_add_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\route_add_order.png)
...
0.0.0.0 是默认路由，指示所有未匹配的流量将通过 10.0.2.2/10.211.55.4 网关发送
 ![route_add_net_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\route_add_net_order.png)

删除网关
route 	del  default(0.0.0.0)	 gw.  10.211.55.1
-net 删除网段 要加上netmask
 ![route_del_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\route_del_order.png)
...
##### 4.网络管理程序（由于使用的是CentOS9 所以这些文件可能不存在）
...
reboot	重启	
 ![hostnamel_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\hostname_order.png)
...
##### 2. rpm 包格式和安装
###### 1.光驱
软件包在光驱里面，光驱在/dev/sr0里面 
是个b_是个块文件
 ![dev_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\dev_order.png)
可以使用 dd if=/dev/sr0  of =/xxx/xx.iso 把一个光盘做成光盘镜像
对于块文件我们要使用挂载进行操作
 ![mount_order](D:\go_mathstruct\everyday\zhouThreeJin\linux\image\mount_order.png)
...
更改yum源
1.备份
mv	 /etc/yum.repos.d/CentOS-Base.repo  	/etc/yum.reois.d/CentOS-Ease.repo.backup
接来下 ：wget -O    /etc/yum.repos.d/CentOS-Base.repo  https://mirrors.aliyun.com/repo/CentOS-9.repo  
